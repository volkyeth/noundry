import { InflaterDecoder } from "@/app/nouns/InflaterDecoder";
import { Noun } from "@/app/nouns/LatestNoun";
import { TraitsList } from "@/app/nouns/TraitsList";

export default function Home() {
  return (
    <main className="flex mx-auto min-h-screen max-w-6xl flex-col items-center gap-4 p-8 sm:p-24">
      <h1 className="text-4xl font-bold mb-10">Nouns Assets</h1>
      <div className="flex flex-col items-center mb-12">
        <p className="text-center max-w-lg mb-4">
          This Noun is generated from the current Noundry-maintained Nouns asset
          snapshot, using the last trait included for each part.
        </p>
        <Noun />
        <p className="text-center max-w-lg mt-4">
          It updates when Noundry refreshes and redeploys the hosted asset data.
          If it looks outdated, reach out to Volky to update it.
        </p>
      </div>

      <h2 className="text-3xl font-bold mb-6">All Nouns Traits</h2>
      <p className="text-center max-w-2xl mb-8">
        Below is a complete list of all Nouns traits included in the current
        asset snapshot.
      </p>

      <TraitsList />

      <h2 className="text-3xl font-bold mb-6 mt-16">
        Trait Inflater & Decoder
      </h2>
      <p className="text-center max-w-2xl mb-8">
        Enter compressed trait data to decode and render traits. You can use
        this to double check the transactions in proposals that add traits or
        update them.
      </p>

      <InflaterDecoder />
    </main>
  );
}
