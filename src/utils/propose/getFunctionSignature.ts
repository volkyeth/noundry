...
export const getFunctionSignature = (func: AbiFunction) => {
  return `${func.name}(${func.inputs.map(getType).join(",")})`;
};

const getType = (parameter: AbiParameter): string => (isTuple(parameter) ? `(${parameter.components.map(getType).join(",")})` : parameter.type);
