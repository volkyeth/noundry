import { Button, Kbd, Modal, ModalBody, ModalCloseButton, ModalContent, ModalHeader, ModalOverlay, SimpleGrid, Text, VStack } from "@chakra-ui/react";
    ...
    <>
      ...
      <Modal isOpen={isOpen} onClose={toggle} size={"6xl"}>
        ...
        <ModalContent maxH={"80vh"} overflowY={"scroll"}>
          ...
          <ModalBody p={10}>
            <SimpleGrid columnGap={8} rowGap={6} templateColumns={"auto 1fr auto 1fr"} alignItems={"center"}>
              {mode.keyBindings.map(({ commands, description }, i) => (
                <>
                  <VStack alignItems={"end"}>
                    {commands.map((command) => (
                      <Kbd>{command.toUpperCase()}</Kbd>
                    ))}
                  </VStack>
